...
def reset_person_inputs(cfg):
    """
    Resets ONLY per-person fields in the "picture" you sent:
...
# RTL styling (Streamlit limitation: sidebar stays left, but content is RTL)
st.markdown(
    """
    <style>
...
if st.sidebar.button("שכפל חודש קודם"):
    y = int(cfg["year"])
    m = int(cfg["month"])
    if m == 1:
        py, pm = y - 1, 12
    ...
    if os.path.exists(candidate):
        ...
        base["month"] = m
        cfg = base
        st.sidebar.success("שוכפל חודש קודם ✅")
    else:
...
with tab_generate:
    ...
    if st.button("⚙️ צור/י לוח עבודה"):
        save_cfg(cfg, "input.json")
        ...
        if status != "OK":
            ...
            st.session_state.diff_df = None  # reset diff for a fresh schedule
            st.success("הלוח נוצר בהצלחה ✅ (הקבצים נשמרו + זמינים להורדה כאן)")

    st.divider()

    # Sick leave re-schedule section
    ...
    if not active_people:
        ...
        if st.button("🔁 צור/י שיבוץ מעודכן (מחלה)"):
            if not st.session_state.last_rows:
                ...
                prev_rows = st.session_state.last_rows  # old schedule for diff
