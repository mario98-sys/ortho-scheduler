...
with qa1:
    if st.button("⚙️ צור/י לוח עבודה", use_container_width=True):
        ...
        if status != "OK":
            ...
            st.success("הלוח נוצר בהצלחה ✅ (זמין להורדה למטה)")
with qa2:
    if st.session_state.generated and st.session_state.excel_bytes:
        st.download_button(
            "⬇️ Excel",
    ...
    if st.session_state.generated and st.session_state.csv_bytes:
        st.download_button(
            "⬇️ CSV",
...
# Sidebar (Advanced)
# -----------------------------
st.sidebar.header("הגדרות (מתקדם)")

cfg["year"] = int(st.sidebar.number_input("שנה", min_value=2000, max_value=2100, value=int(cfg.get("year", 2026))))
cfg["month"] = int(st.sidebar.number_input("חודש", min_value=1, max_value=12, value=int(cfg.get("month", 1))))

st.sidebar.subheader("חוקים")
r = cfg["rules"]
r["no_two_roles_same_day"] = st.sidebar.checkbox("אין 2 עמדות באותו יום לאותו אדם", value=bool(r.get("no_two_roles_same_day", True)))
r["first_second_blocks_next_day_first_second"] = st.sidebar.checkbox("תורן 1/2 חוסם תורן 1/2 למחרת", value=bool(r.get("first_second_blocks_next_day_first_second", True)))
r["first_second_blocks_next_day_maccabi"] = st.sidebar.checkbox("תורן 1/2 חוסם מכבי למחרת", value=bool(r.get("first_second_blocks_next_day_maccabi", True)))
r["max_consecutive_days"] = int(st.sidebar.number_input("מקסימום ימים רצופים", min_value=1, max_value=10, value=int(r.get("max_consecutive_days", 2))))
r["min_rest_days_any"] = int(st.sidebar.number_input("מינימום ימי מנוחה בין משמרות (0=אין)", min_value=0, max_value=7, value=int(r.get("min_rest_days_any", 0))))
cfg["rules"] = r

st.sidebar.subheader("תבניות")
template_files = sorted([f for f in os.listdir(TEMPLATES_DIR) if f.endswith(".json")])
selected_template = st.sidebar.selectbox("בחר תבנית לטעינה", ["(ללא)"] + template_files)
    ...
    if st.button("טען תבנית"):
        if selected_template != "(ללא)":
            with open(os.path.join(TEMPLATES_DIR, selected_template), "r", encoding="utf-8") as f:
                cfg = json.load(f)
            st.sidebar.success("נטענה תבנית ✅")

with col_t2:
    tpl_name = st.sidebar.text_input("שם לשמירה", value=f"schedule_{cfg['year']}_{cfg['month']:02d}")
    if st.button("שמור תבנית"):
        if tpl_name.strip():
            path = os.path.join(TEMPLATES_DIR, f"{tpl_name.strip()}.json")
            with open(path, "w", encoding="utf-8") as f:
                json.dump(cfg, f, ensure_ascii=False, indent=2)
            st.sidebar.success("נשמרה תבנית ✅")

# -----------------------------
# Tabs
# -----------------------------
tab_people, tab_locks, tab_generate = st.tabs(["מתמחים (מובייל)", "נעילות", "תצוגה/בדיקות"])

# -----------------------------
# People tab (mobile friendly)
# -----------------------------
with tab_people:
    st.subheader("עריכת מתמחים (אחד-אחד)")

    if st.button("➕ הוסף/י מתמחה חדש", use_container_width=True):
        cfg["people"].append({
        ...
        save_cfg(cfg, "input.json")
        st.success("נוסף מתמחה ✅")

    if not cfg["people"]:
        st.info("אין מתמחים עדיין.")
    else:
        names = [p.get("name", f"#{i+1}") for i, p in enumerate(cfg["people"])]
        idx = st.selectbox("בחר/י מתמחה לעריכה", range(len(names)), format_func=lambda i: names[i])

        p = cfg["people"][int(idx)]

        p["name"] = st.text_input("שם", value=p.get("name", ""))
        p["excluded"] = st.checkbox("הוצאה זמנית", value=bool(p.get("excluded", False)))
        ...
        q = p.get("quota", {})
        q_first = st.text_input("יעד תורן ראשון", value=str(q.get("first", "")))
        q_second = st.text_input("יעד תורן שני", value=str(q.get("second", "")))
        q_half = st.text_input("יעד תורן חצי", value=str(q.get("half", "")))
        q_m = st.text_input("יעד מכבי", value=str(q.get("maccabi", "")))
        ...
        with b2:
            if st.button("🗑️ מחק מתמחה זה", use_container_width=True):
                cfg["people"].pop(int(idx))
                save_cfg(cfg, "input.json")
                st.warning("נמחק ✅")
                st.rerun()
        ...
        if not st.session_state.reset_pending:
            if st.button("♻️ איפוס (זמינות + יעדים + העדפות) לכל המתמחים", use_container_width=True):
                st.session_state.reset_pending = True
                st.warning(
                    "אזהרה: פעולה זו תאפס עבור כל המתמחים:\n"
                    "- זמינות\n- יעדים (Quota)\n- העדפות כלליות\n- העדפות מכבי\n\n"
                    "לא ישתנה: שם, עמדות, הוצאה זמנית."
                )
        else:
            c1, c2 = st.columns(2)
            with c1:
                if st.button("✅ כן, אפס עכשיו", use_container_width=True):
...
with tab_locks:
    ...
    if not people_names:
        ...
        if st.button("➕ הוסף נעילה", use_container_width=True):
        ...
        for i, lk in enumerate(cfg["locked_assignments"]):
            with st.expander(f"נעילה #{i + 1}", expanded=False):
                ...
                lk["role"] = st.selectbox("עמדה", ROLE_KEYS, index=ROLE_KEYS.index(lk.get("role", "first")),
                                          format_func=lambda r0: ROLE_NAMES_HE[r0], key=f"lkr_{i}")
                ...
                if st.button("🗑️ מחיקת נעילה", key=f"lkdel_{i}"):
                    to_del = i

        if to_del is not None:
            cfg["locked_assignments"].pop(to_del)
            save_cfg(cfg, "input.json")
            st.success("נעילה נמחקה ✅")

        if st.button("💾 שמור נעילות", use_container_width=True):
            save_cfg(cfg, "input.json")
            st.success("נעילות נשמרו ✅")

# -----------------------------
# Generate tab: views + sick reschedule + diff
# -----------------------------
with tab_generate:
    st.subheader("תצוגה / בדיקות / עדכון מחלה")
    ...
    st.markdown("### עדכון לוח במקרה מחלה")
    active_people = [p["name"] for p in cfg.get("people", []) if not p.get("excluded", False)]
    if not active_people:
        st.info("אין מתמחים פעילים.")
    else:
        sick_name = st.selectbox("בחר/י מתמחה שיצא למחלה", active_people)
        from_date = st.number_input("מתאריך", min_value=1, max_value=31, value=15, step=1)
        strict_mode = st.checkbox("מצב קשיח (לשמור ככל האפשר על שיבוץ קיים בעתיד)", value=True)

        if st.button("🔁 צור/י שיבוץ מעודכן (מחלה)", use_container_width=True):
    ...
    if st.session_state.generated:
        tab_table, tab_daily, tab_person, tab_val, tab_diff = st.tabs(
            ["טבלה", "תצוגת יום", "תצוגת מתמחה", "בדיקות", "שינויים"]
        )
        ...
        with tab_daily:
            st.markdown("בחר/י תאריך כדי לראות יום בצורה נוחה למובייל")
            df = st.session_state.schedule_he.copy()
            dates = df["תאריך"].tolist()
            dt = st.selectbox("תאריך", dates)
            row = df[df["תאריך"] == dt].iloc[0].to_dict()
            st.info(f"**{row['תאריך']} — {row['יום']}**")
            st.write(f"**תורן ראשון:** {row.get('תורן ראשון','')}")
            st.write(f"**תורן שני:** {row.get('תורן שני','')}")
            st.write(f"**תורן חצי:** {row.get('תורן חצי','')}")
            st.write(f"**מוקד מכבי:** {row.get('מוקד מכבי','')}")

        with tab_person:
            st.markdown("בחר/י מתמחה כדי לראות רק את המשמרות שלו/שלה")
            df = st.session_state.schedule_he.copy()
            all_names = sorted(set(
                [x for col in ["תורן ראשון","תורן שני","תורן חצי","מוקד מכבי"]
                 for x in df[col].dropna().astype(str).tolist()]
            ))
            who = st.selectbox("מתמחה", all_names)
            mask = (
                (df["תורן ראשון"] == who) |
                (df["תורן שני"] == who) |
                (df["תורן חצי"] == who) |
        ...
        st.info("אחרי יצירת לוח עבודה – תופיע כאן התצוגה (טבלה/יום/מתמחה/בדיקות/שינויים).")
